...
function setToken(res, code) {
  res.setHeader('Set-Cookie', `${COOKIE}=${encodeURIComponent(createToken(code))}; Max-Age=${MAX_AGE}; Path=/; HttpOnly; SameSite=Lax${process.env.NODE_ENV === 'production' ? '; Secure' : ''}`);
}

module.exports = { createToken, readToken, setToken, COOKIE };
