...
const guides = {
  ...
  '动漫像素':'这是严格的原图转动漫像素，不是重新设计或自由创作。以输入原图为唯一构图和内容依据，先锁定人物的头部位置、身体比例、姿势、手势、服装、背包、手中牌子和背景关系，再把它们转换成经典日系动漫像素角色立绘。人物必须保持原图的半身或全身取景、朝向、动作和相对位置，不能变成头像，不能补出原图没有的身体，不能改变牌子形状或把一个人物拆成两个人。画面使用清晰的方形像素、深色干净线稿、明快饱和配色、分组发丝、动漫脸型、大而清楚的眼睛与瞳孔高光、明确的眉毛鼻子嘴巴和多层脸部阴影；重要道具和文字区域只做像素化保留，不要擅自改写。目标是参考图那种完整、可辨认的动漫像素人物，而不是照片马赛克、模糊色块或通用卡通人。禁止写实照片感、油画感、3D渲染、塑料质感、无五官平面脸、随机新增人物、重复主体、分屏和改变道具。',
