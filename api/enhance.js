...
const guides = {
  '精致像素':'忠实原图的精致像素化肖像，不要动漫化、不要Q版化、不要改变人物身份和脸型。严格保留原图中的五官位置和比例：眼睛、眉毛、鼻子、嘴唇、脸部轮廓必须与原图一致；保留原图发型、发色、肤色、耳饰、服装、姿势和头部到胸口的构图。使用细腻但清晰的像素块表现原图的光影和材质，五官要有独立的眼白、瞳孔、鼻梁、鼻尖和嘴唇层次，不要把脸处理成动漫大眼或扁平卡通脸。背景尽量沿用原图的深色大色块和明暗关系，去掉文字水印和无关细碎纹理，但不要改成明亮插画背景。颜色以原图为准，只做必要的像素化明暗过渡，不凭空增加鲜艳颜色。主体完整、清晰、居中，不能只生成脸部特写，也不能丢失衣服和肩膀。',
  ...
  '头像像素':'高保真头像像素化，只保留原图中的头部和肩部，脸部占画面约55%到65%，保持自然头肩比例。严格按照原图保留本人原有脸型、五官比例、眼睛形状和位置、眉毛、鼻梁鼻尖、嘴唇、眼镜、发型、发色、肤色、表情和衣服；只做像素化，不重新设计、不夸张放大、不换脸。脸部必须有清晰的眼睛、眉毛、鼻子和嘴巴轮廓，用多个明暗色块表现五官结构，头发按原图发型分组，不能吞掉额头和眼睛。背景必须彻底简化为单一纯色或最多两块平整大色块，禁止保留墙面纹理、灯光、景物、杂物和细碎背景细节。禁止夸张动漫五官、过大头部、细长脸、磨皮、改变年龄、改变发型、添加配饰、重复头像、分屏和文字水印。宠物同样保留真实脸型、毛色、眼睛、鼻子、胡须和耳朵。',
  'Q版像素':'高完成度Q版动漫像素角色：头部适度放大、脸部占主体、圆润脸型，使用清晰的大眼睛、瞳孔高光、眉毛、鼻子和嘴巴，五官之间保留明显深色像素轮廓和明暗层次。保留原图人物的发型、表情、姿势、服装和主要道具，衣服简化为清楚的大色块，背景降低杂色。不要生成模糊色块、无五官平面脸、重复人物、分屏或裁切。颜色必须严格参考原图，尤其是肤色保持自然的米色、浅桃色或原图肤色；禁止把整张脸染成红色、橙红色或鲜粉色，红色仅可用于嘴唇和极少量自然腮红，脸部阴影使用原图对应的柔和浅棕或灰粉色，不要凭空增加颜色。',
...
guides['动漫像素'] += ' 背景必须进一步压缩：只保留与构图有关的轮廓和大色块，去掉墙面纹理、细碎装饰、零散高光、重复线条和无关小物；背景颜色控制在2到4个主色区域，人物边缘与背景有清楚对比。';
guides['邮票像素'] += ' 人物肤色必须自然偏米色或浅桃色，禁止整张脸使用鲜红、深粉或橙红；红色只用于嘴唇、极少量腮红和服装细节，脸部阴影使用柔和的浅棕或灰粉色。';
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `${guides[req.body.style] || guides['精致像素']} 这是拼豆图纸的第一阶段：先生成一张干净、清晰、可继续量化的像素原稿，不要直接生成拼豆网格。采用主体优先构图：让原图中最重要的人物或动物完整、清晰地占画面约70%到85%，减少天空、地面和杂乱背景的像素占比；主体不能被裁掉头部、四肢或关键服装。先识别原图构图，再转换成忠实原图的像素原稿。保留原图中所有实际可见的主要主体和相对位置，但每个主体只出现一次；禁止重复主体、大头照与全身照并列、双视图、分屏、拼贴、对照图、不同角度展示或上下两部分画面。不要把横图强行变成只剩头像，也不要补出原图之外的身体或场景。背景简化为少量大色块，不要让背景抢主体。不要生成网格、色号、文字或水印。保持人物和宠物真实特征，色块成片，五官清晰。颜色必须严格对齐原图：优先使用原图中已经存在的主色和相近明暗，不凭空添加荧光色、鲜艳腮红或不属于原图的高光颜色；每个区域只做必要的像素化明暗过渡。`;
